<x-app-layout>
    <div class="py-12 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <a href="{{ route('operator.dashboard') }}" class="inline-flex items-center text-slate-500 hover:text-blue-600 font-bold mb-6 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-1">
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                        <h3 class="text-xl font-black text-gray-800 mb-6">📅 Input Jadwal Jaga</h3>
                        <form action="{{ route('operator.schedules.store') }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Driver / Nakes</label>
                                <select name="user_id" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:ring-blue-500" required>
                                    <option value="">-- Pilih Petugas --</option>
                                    @foreach($drivers as $driver)
                                        <option value="{{ $driver->id }}">{{ $driver->name }} ({{ ucfirst($driver->role) }})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="2" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:ring-blue-500"></textarea>
                            </div>
                ...
                <div class="md:col-span-2">
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                        <h3 class="text-xl font-black text-gray-800 mb-6">List Jadwal Minggu Ini</h3>
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    @forelse($schedules as $schedule)
                                        <tr class="hover:bg-slate-50 transition">
                                            ...
                                            <td class="px-4 py-4 font-bold text-gray-800">{{ $schedule->user->name }}</td>
                                            <td class="px-4 py-4">
                                                @php
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-8 text-gray-400">Belum ada jadwal dibuat.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
